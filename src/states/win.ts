...
export async function win(this: Game,canvas: HTMLCanvasElement, engine: Engine, status: Status, gameStatus:GameStatus){
...
async function makeGui(sceneToLoad: Scene, gameStatus: GameStatus) {
    ...
    const congratsText = await simpleTextBlock("congratsText","Congratulations","white",fontSizePercentage/1.5,0.5,0,Control.VERTICAL_ALIGNMENT_CENTER);
    congratsText.textWrapping = true;
    grid.addControl(congratsText,0,0);

    const scoreText = await simpleTextBlock("scoreText","You found "+gameStatus._hud.NpcCount + " / 10 rabbits in less than one minute","white",fontSizePercentage/2,0.5,0,Control.VERTICAL_ALIGNMENT_CENTER);
